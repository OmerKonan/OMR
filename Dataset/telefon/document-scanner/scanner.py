...
image = cv2.resize(image, (1700, 2340))
...
dst = cv2.warpPerspective(orig,M,(x,y))
...
th1 = cv2.threshold(dst,135,255,cv2.THRESH_BINARY)[1]
...
th3 = cv2.adaptiveThreshold(dst,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
            cv2.THRESH_BINARY,11,2)
th4 = cv2.threshold(dst,150,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1]
th5 = cv2.threshold(dst,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1]
...
ret,thresh3 = cv2.threshold(dst,200,255,cv2.THRESH_TRUNC)
th4 = cv2.threshold(thresh3,130,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1]
ret,thresh4 = cv2.threshold(dst,127,255,cv2.THRESH_TOZERO)
ret,thresh5 = cv2.threshold(dst,127,255,cv2.THRESH_TOZERO_INV)

#cv2.imwrite("./data/test2.jpg",dst)
print("saved")
